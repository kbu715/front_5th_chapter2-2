...
describe("advanced > ", () => {
  ...
  describe("자유롭게 작성해보세요.", () => {
    ...
    test("getCouponAppliedTotal 유틸 함수를 이용해 금액 할인 쿠폰을 적용한 결과를 확인한다.", () => {
      const discountAmount = getCouponAppliedTotal(originalPrice, amountCoupon);
      expect(discountAmount).toBe(5000);
    });

    test("getCouponAppliedTotal 유틸 함수를 이용해 퍼센트 할인 쿠폰을 적용한 결과를 확인한다.", () => {
      const discountAmount = getCouponAppliedTotal(
        originalPrice,
        percentageCoupon
      );
      expect(discountAmount).toBe(5000);
    ...
    test("useProducts hook의 removeProduct 함수를 이용해 상품을 삭제한다.", () => {
      const productsList = mockProducts;
      const { result } = renderHook(() => useProducts(productsList));

      act(() => {
        result.current.removeProduct(mockProducts[0]);
      });
